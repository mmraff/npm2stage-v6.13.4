...
function expectNoLeftovers() {
  ...
  // Sadly, putting 'istanbul ignore next' before that last call does not
  // get the expected behavior; instead, we have to ignore every line in
  // that function.
}
...
function copyFilesFromCWD(list, dest) {
  function nextItem(i) {
    ...
    .catch(err => {
      ...
      // TODO: if coverage tests are implemented, this should be skipped
      // UNLESS we keep the test that temporarily renames a file in src.
...
module.exports.install = function(npmDir) {
  const startDir = process.cwd()
  const srcDir = path.join(path.dirname(__dirname), 'node_modules/npm-two-stage/src')
  ...
  .then(() => {
    ...
    try { process.chdir(srcDir) }
      ...
      // TODO: if coverage tests are implemented, this should be skipped,
      // unless we keep the test that temporarily renames the src directory.
      err.exitcode = ERRS.BAD_PROJECT
      throw err
    }
